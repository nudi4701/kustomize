pipeline {
  ...
  environment {
    ...
    awsecrRegistry = '975049987503.dkr.ecr.ap-northeast-2.amazonaws.com/eks-demo-repo'
    awsecrRegistryCredential = 'credential-AWS-ECR'
    githubCredential = 'credential-github'
    gitEmail = 'go4701@gmail.com'
    gitName = 'nudi4701'
  }
  ...
  stages {
    ...
    stage('Checkout Application Git Branch') {
      steps {
        checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: githubCredential, url: 'https://github.com/nudi4701/kustomize.git']]])
      }
      // steps 가 끝날 경우 실행한다.
    ...
    stage('Deploy') { 
      steps {
        ...
        git credentialsId: githubCredential,
            url: 'https://github.com/nudi4701/kustomize.git',
            branch: 'main'  
    ...
    stage('Push to Git Repository') {
      steps {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: githubCredential, usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD']]) {
             sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/nudi4701/kustomize.git"       
        }
      }
    }
  }
